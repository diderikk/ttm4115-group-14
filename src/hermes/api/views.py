...
def deliver(request):
    ...
    if request.method == 'POST':
        ...
        delivery = Delivery.objects.create_delivery(
            file=f, group=group, task=task)
        time.sleep(0.5)
        s.trigger(uuid=state_cookie, trigger='complete',
                  kwargs={'request': request, 'task': task})
...
def tasks(request):
    state_cookie = request.COOKIES.get("STATE_COOKIE")
    if request.method == 'POST':
        t.trigger(uuid=state_cookie, trigger='publish_task', kwargs={'request': request})
        time.sleep(0.5)
        return JsonResponse({}, status=201)
    elif request.method == 'GET':
        ...
        if group or is_admin:
            if email and password:
                user = User.objects.create_user(
                    email=email, password=password, is_admin=is_admin, group=group)
